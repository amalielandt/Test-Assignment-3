...
@Tag("integration")
public class GetBookingsTest extends ContainerizedDbIntegrationTest {
    ...
    @Test
    public void mustRetrieveBookingsFromDatabaseWhenCallingGetBookingsForCustomer() throws SQLException, ParseException {
        // Arrange
        Faker faker = new Faker();
        Date date = faker.date().between(new Date(), DateUtils.addYears(new Date(), 1));
        // Act
        int id = bookingStorage.createBooking(new BookingCreation(2, 3,
                dateFormatter.format(date),
                timeFormatter.format(date),
                timeFormatter.format(DateUtils.addHours(date,2))));

        // Assert
        Collection<Booking> bookings = bookingStorage.getBookingsForCustomer(2);
    ...
    @Test
    public void mustRetrieveBookingsFromDatabaseWhenCallingGetBookingsForEmployee() throws SQLException, ParseException {
        // Arrange
        Faker faker = new Faker();
        Date date = faker.date().between(new Date(), DateUtils.addYears(new Date(), 1));
        // Act
        ...
        // Assert
        Collection<Booking> bookings = bookingStorage.getBookingsForEmployee(3);
